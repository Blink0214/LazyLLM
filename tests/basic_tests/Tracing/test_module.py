...
def test_tool_manager_module_tracing(exporter):
    ...
    assert spans[0].attributes.get('lazyllm.entity.class') == 'ToolManager'
...
def test_online_embedding_module_tracing(exporter):
    with patch.object(OnlineEmbeddingModule, 'forward', return_value=[[0.1, 0.2], [0.3, 0.4]]):
